...
try:
    from vac2fost.vac2fost import Interface, MCFOSTUtils, twoD2threeD
    skipme = False
except ImportError:
    skipme = True
...
@pytest.mark.skipif(skipme, reason="refactoring...")
class Test3DConv:
    def test_3D_conversion_cst_scale_height(self):
        #radial powerlaw
    ...
    def test_3D_conversion_var_scale_height(self):
        #radial powerlaw
        input_data = rgrid**-0.5
        h = 0.05 * rgrid
    ...
    def test_3D_conversion_large_grid(self):
        NR, NPHI, NZ = 23, 61, 7
        rvect = np.linspace(1, 10, NR)
        phivect = np.linspace(0, 2*np.pi, NPHI)
        zvect = np.linspace(0, 1, NZ)
    ...
    def test_3D_conversion_real_usecase(self):
