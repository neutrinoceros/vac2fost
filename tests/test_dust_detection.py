...
class TestDBM:
    ...
    def test_unrecognized_dbm(self):
        with pytest.raises(ValueError):
            Interface(
                TEST_VTU_DATA_DIR / "vac2fost_conf_quick_no_dust.nml",
    ...
    def test_conflict_conffile(self):
        with pytest.raises(RuntimeError):
            Interface(
                TEST_VTU_DATA_DIR / "vac2fost_conf_conflict_g2d_ratio.nml",
                output_dir=output_dir,
            )
    ...
    def test_auto_dbm(self, conf_file, expected):
        itf = Interface(TEST_VTU_DATA_DIR / conf_file, output_dir=output_dir)
